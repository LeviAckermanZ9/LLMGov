...
@pytest.fixture
def mock_litellm_toxic():
    mock_response = MagicMock()
    mock_response.id = "mock-id-toxic"
    mock_response.created = 1234567890
    mock_response.model = "gemini/gemini-2.5-flash"
    mock_choice = MagicMock()
    mock_choice.index = 0
    mock_choice.message.role = "assistant"
    mock_choice.message.content = "You are an asshole."
...
def test_toxicity_classified_on_cache_miss(mock_litellm_toxic, mock_embedding, mock_telemetry, mock_redis_client, auth_headers):
    with patch("app.api.completions.authenticate_api_key", AsyncMock(return_value="dev_app")), \
         ...
         patch("app.api.completions.classify_toxicity", return_value=(0.8, True)) as mock_classify:
        ...
        # Verify classify_toxicity was called with the model's generated content
        mock_classify.assert_called_once_with("You are an asshole.")
